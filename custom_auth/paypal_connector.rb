{
    ...
    fields: [
      {
        ...
        control_type: :select,
        pick_list: [
          %w[Production paypal.com],
          %w[Sandbox sandbox.paypal.com]
        ]
      },
      {
        name: "client_id",
  ...
  actions: {
    search_invoices: {
      title: "Search invoices in PayPal",
  ...
  triggers: {
    new_invoice: {
      title: "New invoice in PayPal",
      description: "New <span class='provider'>invoice</span> in "\
        "<span class='provider'>PayPal</span>",
  ...
  pick_lists: {
    ...
    status_list: lambda do
      [
        ...
        %w[Sent SENT],
        %w[Paid PAID],
        %w[Mark\ as\ Paid MARKED_AS_PAID],
        %w[Cancelled CANCELLED],
        %w[Refunded REFUNDED],
        %w[Partially\ Refunded PARTIALLY_REFUNDED],
        %w[Mark\ as\ Refunded MARKED_AS_REFUNDED],
        %w[Unpaid UNPAID],
        %w[Payment\ Pending PAYMENT_PENDING]
      ]
    end,
